...
namespace SISKPI {
	...
	/// 数据信息
	/// </summary>
	public partial class KPI_ForShiftValue : System.Web.UI.Page {
		static string WebCode = "";
		static double[] totalvalue = null;
		...
		protected static DataTable ConvertToTable(DataTable source, string calctype) {
			...
			var columns = (from x in source.Rows.Cast<DataRow>() orderby x[1] select x[1] +"").Distinct();
			...
			data.ToList().ForEach(x => {
				...
				if (arraydb.Count == 0) {
					...
					array[dt.Columns.Count - 1] = arraydb[0].ToString("0.000");
				}
				else {
					switch (calctype) {
						//得分
						...
						//累计值
						case "7":
							array[dt.Columns.Count - 1] = arraydb.Sum().ToString("0.000");
						...
						case "6":
							...
							array[dt.Columns.Count - 1] = arraydb.Average().ToString("0.000");
							break;

						//最大值
						case "3":

							array[dt.Columns.Count - 1] = arraydb.Max().ToString("0.000");
							break;

						//最小值
						case "4":

							array[dt.Columns.Count - 1] = arraydb.Min().ToString("0.000");
							break;
		...
		protected void gvData_RowDataBound(object sender, GridViewRowEventArgs e) {
			if (e.Row.RowType == DataControlRowType.DataRow) {
				...
				for (int i = 1; i < totalvalue.Length; i++) {
					double dtemp = 0.0;
					if (double.TryParse(myrows[i].ToString(), out dtemp)) {
						totalvalue[i] += dtemp;
					}
				}
			}
			...
			if (e.Row.RowType == DataControlRowType.Footer) {
				...
				for (int i = 1; i < totalvalue.Length; i++) {
					e.Row.Cells[i + 1].Text = totalvalue[i].ToString();
				}
			}
		}
		#endregion
