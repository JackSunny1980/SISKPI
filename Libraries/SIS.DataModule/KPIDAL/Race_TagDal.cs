...
namespace SIS.DataAccess {
	public class Race_TagDal : DalBase<Race_TagEntity>,IDisposable {
		...
		/// </summary>
		/// <returns></returns>       
		public  List<Race_TagEntity> GetTagList(string UnitID) {
			List<Race_TagEntity> Result = null;
			string sqlText = @"SELECT TagID, TagName, TagDesc, TagEngunit FROM Race_Tag ORDER BY TagIndex";
			if (!String.IsNullOrEmpty(UnitID)) {
				sqlText = string.Format(@"SELECT TagID, TagName, TagDesc, TagEngunit 
								FROM Race_Tag WHERE  UnitID='{0}' ORDER BY TagIndex", UnitID);
			}
			using (IDataReader reader = DBAccess.GetRelation().ExecuteReader(sqlText)) {
				Result = reader.FillGenericList<Race_TagEntity>();
				reader.Close();
			}
			return Result;
		...
		public static string GetTagID(string TagName, string UnitID) {
			Race_TagEntity entity = new Race_TagEntity();
		...
		/// <param name="SetSnapShot">是否设置测点实时值</param>
		/// <returns></returns>
		public static Race_TagEntity GetEntity(string TagID) {
			Race_TagEntity entity = new Race_TagEntity();
		...
		public static bool Delete(string TagID) {
			...
			return Race_TagDal.Delete(entity);
		}
		...
		public static DataTable GetSearchList3(string condition) {
			string sql = @"select TagID, UnitName, UnitDesc, TagName, TagDesc, TagType, TagEngunit, TagIsValid, TagIndex,
                            ...
                            from Race_Tag  
                            a left outer join KPI_Unit b on a.UnitID = b.UnitID
                            where 1=1 {0}  
                            order by TagIndex";
