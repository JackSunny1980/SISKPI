...
namespace SIS.DBControl
{
    ...
    public sealed class OracleHelper 
	{
        ...
        {
            //string conStr = (string)Cache.GetData("oracleConntionString");
            //if (conStr == null || conStr == "")
            //{
            //    /**/
            //    ///如果连接字符串为空，则从配置文件中获取连接字符串
            // string   conStr = System.Configuration.ConfigurationSettings.AppSettings["oracleConntionString"];
            //    /**/
            //    ///把字符串添加到缓存中
            //    Cache.CachingData("oracleConntionString", conStr);
            //}
			string conStr = ConfigurationManager.AppSettings["ConntionString"];
            return conStr;
        }
		...
		public static bool LockTable(OracleTransaction trans,string tablename)
		{
			bool result=false ;

			string SQLString ;
			SQLString = "select LOCK_ID from Dim_LockTable where TABLE_NAME = '"+tablename+"'" ;

			try
			{
				string ID = ExecuteDataset(trans,CommandType.Text,SQLString).Tables[0].Rows[0]["LOCK_ID"].ToString() ;
		...
		private static void PrepareCommand(OracleCommand command, OracleConnection connection, OracleTransaction transaction, CommandType commandType, string commandText, IDbDataParameter[] commandParameters, out bool mustCloseConnection)
		{
			if( command == null ) throw new ArgumentNullException( "command" );
